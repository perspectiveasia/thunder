...
export class ServerlessServer extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ServerlessServerConstructProps) {
    ...
    // If Dockerfile is specified, use container mode, otherwise use ZIP mode
   ...
   * @param {ServerlessServerConstructProps} props - The properties for the app.
  ...
  /**
   * Creates the Lambda function to render the app.
   *
   * @private
   */
  ...
  /**
   * Creates the API gateway to make the app render Lambda function publicly available.
   *
   * @private
   */
